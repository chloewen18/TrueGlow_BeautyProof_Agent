...
def run_ocr(image_path):
    ...
    variants, _ = make_variants(image)
...
def evidence_matches_for_claim(evidence_db, claim_name, product=None):
    ...
    for product_item in evidence_db.get("products", []):
        ...
        if product and product.lower() not in product_name.lower():
            continue

        for item in product_item.get("claims", []):
            if item.get("canonical_claim") == claim_name:
                matches.append({
...
def main():
    ...
    parser.add_argument(
        "--dictionary",
        default="Foundation_Claim_Dictionary_V2_2_Separate_Slang_Exaggerated.xlsx"
    )
    parser.add_argument(
        "--evidence",
        default="Member4_Mini_Efficacy_Evidence_Library_v1.json"
    )
    parser.add_argument(
        "--product",
        default=None,
