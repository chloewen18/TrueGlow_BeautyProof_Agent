...
from api.client import _upload, _result, api_post
...
def verify(text, product, image=None, before=None, comments=""):
    ...
    return _result(api_post("/api/v1/verify",json=payload,timeout=600))

def demo(case):
    result = _result(api_post("/api/v1/verify",
    ...
    return _result(api_post("/api/v1/creators/review",json=payload,timeout=120))

def ocr(image):
    ref = _upload(image)
    response = api_post("/api/v1/member4/ocr",json={"media_ref":ref["ref"]},timeout=240)
    if not response.ok:
